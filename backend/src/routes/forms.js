...
// ✨ NEW: Get list of saved reports for frontend
router.get('/list', async (req, res) => {
    try {
        const limit = parseInt(req.query.limit) || 50;
        logger.info(`Fetching recent forms (limit: ${limit})`);
        
        const forms = await trackingService.getRecentForms(limit);
        
        res.json({
            success: true,
            reports: forms,
            count: forms.length,
            source: forms.length > 0 ? 'database' : 'none',
            message: forms.length > 0 
                ? `Loaded ${forms.length} reports from database`
                : 'No reports found in database'
        });
        
    } catch (error) {
        logger.error('Error fetching forms list:', error);
        res.status(500).json({
            success: false,
            error: 'Failed to fetch reports list',
            details: error.message,
            reports: [] // Return empty array as fallback
        });
    }
});
...
router.post('/:id/pdf', async (req, res) => {
    try {
        ...
        logger.info(`📄 PDF generation request for form ${id}`);

        // Get form from database
        let form;
        try {
            form = await trackingService.getFormById(id);
        ...
        // Transform database form to report format
        ...
        try {
            ...
            logger.info(`✅ PDF generated successfully for form ${id}`);

            // Set headers for PDF download
            res.setHeader('Content-Type', 'application/pdf');
            res.setHeader('Content-Disposition', `attachment; filename="safety-report-${id}.pdf"`);
        ...
        logger.error('PDF endpoint error:', error);
        res.status(500).json({
            success: false,
...
/**
 * POST /api/forms/pdf/generate
 * Generate PDF from in-memory report (for unsaved reports)
 */
router.post('/pdf/generate', async (req, res) => {
    try {
        ...
        logger.info(`📄 Generating PDF for unsaved report: ${report.file}`);
        ...
        // Set headers for PDF download
        const filename = `safety-report-${Date.now()}.pdf`;
        res.setHeader('Content-Type', 'application/pdf');
